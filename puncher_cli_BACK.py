...
def load_used_ids(csv_path: Path, id_var: str) -> set[str]:
    """
    Wczytuje wszystkie dotychczas użyte identyfikatory z responses.csv.
    Zakłada, że kolumna id_var jest nagłówkiem w CSV.
    Jeśli plik nie istnieje albo nie ma takiej kolumny – zwraca pusty zbiór.
    """
    used = set()
...
def warn_duplicate_id(stdscr, value: str):
    ...
    msg1 = " DUPLICATE ID "
    msg2 = f" The ID '{value}' is already used in responses.csv. "
    msg3 = " Please enter a different identifier. "

    lines = [msg1, "", msg2, msg3]
...
def save_answers_to_csv(answers: Dict[str, str], path: str):
    """
    Zapis:
...
def draw_header(stdscr, current_page: int, interview_no: int):
    h, w = stdscr.getmaxyx()
...
def draw_page(
    ...
    draw_header(stdscr, current_page, interview_no)
        ...
        while True:  # pętla w obrębie jednej ankiety
            ...
            if not any(f.active for f in fields):
                if current_page_idx < TOTAL_PAGES - 1:
                    ...
                    save_answers_to_csv(answers, CSV_PATH)
                    interview_no += 1
                    break  # nowa ankieta
...
def main():
    global UNIQUE_ID_VAR, USED_IDS, ITEMS, PAGES_ITEMS, TOTAL_PAGES

    ITEMS = parse_dictionary(DICT_PATH)
    PAGES_ITEMS = split_pages(ITEMS)
    TOTAL_PAGES = len(PAGES_ITEMS)

    # wszystkie pytania (kind == "question"), z nazwą
    question_items = [it for it in ITEMS if it.kind == "question" and it.name]

    if not question_items:
        raise RuntimeError("Dictionary has no question items – cannot determine unique ID variable.")

    var_names = [it.name for it in question_items]

    # pierwsze pytanie będzie identyfikatorem ankiety
    UNIQUE_ID_VAR = var_names[0]

    # wczytujemy dotychczas użyte ID z CSV
    USED_IDS = load_used_ids(CSV_PATH, UNIQUE_ID_VAR)
